...
jobs.forEach((job, i) => {
  ...
  card.innerHTML = `
    ...
    <div class="job-actions">
      <button class="small-btn primary-sm details-btn">Xem chi tiết</button>
      <a class="small-btn" href="https://t.me/S8HR1" target="_blank" rel="noopener">Ứng tuyển</a>
    </div>
  `;
...
if (applyForm) {
  applyForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!res.ok || !result.ok) throw new Error(result.message || "Không thể gửi hồ sơ.");
      formStatus.textContent = "✓ Đã gửi hồ sơ thành công. HR sẽ liên hệ bạn sớm.";
      applyForm.reset();
    } catch (err) {
